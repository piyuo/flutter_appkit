...
void main() {
  ...
  group('[filter]', () {
    ...
    test('should filter keyword', () async {
      ...
      await filter.insert([sample.Person(entity: pb.Entity(id: 'third'))]);
      expect(filter.length, 1);
      expect((await filter.first)!.entityID, 'first');
      await filter.insert([sample.Person(entity: pb.Entity(id: 'first2'))]);
      expect(filter.length, 2);
      expect((await filter.first)!.entityID, 'first2');

      // show keep filter when add new data
      await filter.add([sample.Person(entity: pb.Entity(id: 'thirdAdd'))]);
      expect(filter.length, 2);
      expect((await filter.first)!.entityID, 'first2');
      await filter.add([sample.Person(entity: pb.Entity(id: 'first3'))]);
