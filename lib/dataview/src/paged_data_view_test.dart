  ...
  group('[data.paged_data_view]', () {
      ...
      final dataView = PagedDataView<sample.Person>(
        ...
        loader: (isRefresh, limit, anchorTimestamp, anchorId) async {
          ...
          return List.generate(
            limit,
            (index) => sample.Person(m: net.Model(i: idCount.toString())),
          );
        },
      );
      await dataView.load();
    ...
    test('should select rows', () async {
      DataView dataView = PagedDataView(
        DatasetRam(objectBuilder: () => sample.Person()),
        loader: (isRefresh, limit, anchorTimestamp, anchorId) async {
          return List.generate(10, (i) => sample.Person(m: net.Model(i: '$i')));
        },
      );
      await dataView.load();
      await dataView.refresh();
      ...
      dataView.setSelectedRows([sample.Person(m: net.Model(i: '5'))]);
      ...
      final dataView = PagedDataView<sample.Person>(
        ...
        loader: (isRefresh, limit, anchorTimestamp, anchorId) async {
          ...
          if (step == 1) {
            // first more
            step++;
            return List.generate(limit, (index) => sample.Person(m: net.Model(i: 'firstMore$index')));
          }
          if (step == 2) {
            // second refresh
            step++;
            return List.generate(2, (index) => sample.Person(m: net.Model(i: 'secondMore$index')));
          }
          return [];
        },
      );
