...
void main() {
  group('[data.filtered_dataset]', () {
    ...
    test('should filter keyword', () async {
      ...
      await filter.insert([sample.Person(m: net.Model(i: 'third'))]);
      expect(filter.length, 1);
      expect((await filter.first)!.id, 'first');
      await filter.insert([sample.Person(m: net.Model(i: 'first2'))]);
      expect(filter.length, 2);
      expect((await filter.first)!.id, 'first2');

      // show keep filter when add new data
      await filter.add([sample.Person(m: net.Model(i: 'thirdAdd'))]);
      expect(filter.length, 2);
      expect((await filter.first)!.id, 'first2');
      await filter.add([sample.Person(m: net.Model(i: 'first3'))]);
