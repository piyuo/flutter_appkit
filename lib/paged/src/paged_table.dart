...
class PagedTable<T> extends StatelessWidget {
  ...
  bool get isTableLayout {
    if (rowBuilder != null && cardBuilder != null) {
      return delta.isPhone ? false : true;
    }
    if (cardBuilder != null) {
      return false;
    }
        ...
        return provide.isBusy
            ...
            : buildInfo(
                InkWell(
                  ...
                  onTap: () => provide.loadMoreRow(context),
                ),
              );
  ...
  Widget buildFooter(
    ...
    final pageWidgets = <Widget>[
      ...
      if (dataSource.status == PagedDataSourceStatus.end)
        IconButton(
  ...
  Widget buildHeader(BuildContext context, PagedDataSource dataSource, double paddingLeft, delta.TapBreaker breaker) {
    ...
    if (dataSource.selectedRowCount == 0 && header != null) {
      ...
      headerWidgets.add(
        Expanded(
          child: Padding(
            ...
            child: Text(localizations.selectedRowCountTitle(dataSource.selectedRowCount)),
          ),
        ),
      );
    }
    ...
    return headerWidgets.isNotEmpty
        ? Semantics(
            ...
            child: DefaultTextStyle(
              ...
              style: dataSource.selectedRowCount > 0
              ...
              child: IconTheme.merge(
                ...
                child: Ink(
                  height: 64.0,
                  color: dataSource.selectedRowCount > 0 && isTableLayout ? themeData.secondaryHeaderColor : null,
                  child: Padding(
  ...
  List<DataRow> buildRows(BuildContext context, PagedDataSource provide, RowBuilder<T> builder) {
    return List<DataRow>.generate(
      provide.rowCount,
      (int position) {
        final rowIndex = provide.rowIndex(position);
        final row = provide.row(rowIndex);
        return DataRow(
          selected: provide.isSelected(rowIndex),
          onSelectChanged: (bool? selected) => provide.select(rowIndex, selected),
  ...
  Widget _buildTable(BuildContext context, PagedDataSource provide) {
    return DataTable(
      ...
      rows: buildRows(context, provide, rowBuilder!),
    );
  }
  ...
  Widget _buildCard(BuildContext context, PagedDataSource provide, double maxWidth, delta.TapBreaker breaker) {
    ...
    return Theme(
        ...
        child: DataTable(
          ...
          headingRowColor:
              dataSource.selectedRowCount > 0 ? MaterialStateProperty.all(themeData.secondaryHeaderColor) : null,
          columns: [
            DataColumn(
                label: SizedBox(
              width: cardWidth,
